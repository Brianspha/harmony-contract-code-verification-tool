...
const fs = require("fs")
async function clone(githubUrl, directory) {
  ...
  rimraf.sync(path.join(path.resolve(directory)));
  const actualUrl = githubUrl.split("/blob")[0];
  fs.mkdirSync(path.join(path.resolve(directory)),{recursive:true});
  execSync(`git clone ${actualUrl} ${directory}`);
}

async function getCommitHash(directory) {
